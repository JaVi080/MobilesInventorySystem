 ...
 async function cities_supp() {
    const res = await secureFetch('http://localhost:5000/api/Suppliers_View?param=all');
...
const act_form=document.getElementById("Supplier_form")?.addEventListener("submit",async (e)=>{
    ...
    try{
      const res=await secureFetch('http://localhost:5000/api/AddSuppliers',{
          method:"POST",
...
async function update_database_supplier(supplier_id, data) {
  try {
    const res = await secureFetch('http://localhost:5000/api/UpdateSuppliers', {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
async function viewData(str) {
  try {
    const res_get = await secureFetch(`http://localhost:5000/api/Suppliers_View?param=${str}`);
...
searchBtn?.addEventListener("click", async () => {
  try {
    ...
    const res_get = await secureFetch('http://localhost:5000/api/Suppliers_View');
    const res_data = await res_get.json();
