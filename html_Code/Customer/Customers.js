...
const act_form = document.getElementById("Customer_form")?.addEventListener("submit", async (e) => {
  ...
  try {
    const res = await fetch('http://localhost:5000/api/AddCustomers', {
      method: "POST",
...
async function update_database_customer(customer_id, data) {
  try {
    const res = await fetch('http://localhost:5000/api/UpdateCustomers', {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
async function viewData(str) {
  try {
    const res_get = await fetch(`http://localhost:5000/api/Customers_View?param=${str}`);
...
searchBtn?.addEventListener("click", async () => {
  try {
    ...
    const res_get = await fetch('http://localhost:5000/api/Customers_View?param=all');
    const res_data = await res_get.json();
